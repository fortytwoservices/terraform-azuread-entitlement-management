    ...
    for catalog in var.entitlement_catalogs : [                                                                                              # Iterates through all Entitlement Catalogs
      for ap in catalog.access_packages : [                                                                                                  # Iterates through all Access Packages within each Catalog
        for resource in ap.resources : merge(resource, {                                                                                     # Iterates through all Resources within each Access Package, within each Catalog
          catalog_key                             = replace(catalog.display_name, " ", "-")                                                  # Creates a reference key to the Entitlement Catalog
          access_package_key                      = replace("${catalog.display_name}-${ap.display_name}", " ", "-")                          # Creates a reference key to the Access Package
          access_package_resource_association_key = replace("${catalog.display_name}-${ap.display_name}-${resource.display_name}", " ", "-") # Creates a reference key for the Access Package Resource Associations
          catalog_resource_association_key        = replace("${catalog.display_name}-${resource.display_name}", " ", "-")                    # Creates a reference key to be used for the Catalog Resource Associations
        })                                                                                                                                   # As keys are based on display names, spaces are removed from the names
      ]
    ]
  ])
